...
HTTP 接缝：`ChangeCuratedDraftHttpAcceptanceTest`。已接受处理人 `POST /api/conflicts/{id}/branch-selection`（`CHANGE_CURATED_TO_OBSERVED`）生成开放草案；`GET /api/conflicts/{id}/curated-drafts/open` 条目 ≥2（X/Y `运行于` A→B，PENDING）。选支后「应该在哪」仍为 A，活跃计划 `PLAN_NOT_FOUND`，事件 `DRAFT_CREATED`。Flyway V13 草案表；`FIX_ACTUAL` 仍跳过草案出计划。无按条写入策展。
