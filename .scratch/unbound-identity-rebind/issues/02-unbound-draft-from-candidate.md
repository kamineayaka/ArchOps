...
- [ ] `UNKNOWN_OBJECT_ID` 候选发草案 → ≥2 条：新建策展容器（不可变标签 = 现场标签）+ 策展 `运行于` 候选所在宿主；接受前 GET 策展没有该新对象，「应该在哪」不变
- [ ] 身份失联 + `MISSING_LABEL`（或错标）候选发草案 → ≥2 条互斥：绑到已有失联对象 vs 新建
- [ ] 草案不出现在冲突下的改理想草案 API 上；无活跃操作计划因本发起而产生
- [ ] 未认证调用发起被拒绝；已认证一般角色与高级角色均可发起
- [ ] 同一 `sourceHostId` + `runtimeId` 第二份开放草案被拒绝
- [ ] HTTP 可读开放草案及其 PENDING 条目；无「整单全接受」
...
Failure (witnessed): Status expected:<200> but was:<500>; No static resource api/observed/unbound-candidates/.../drafts (NoResourceFoundException → INTERNAL_ERROR). Honest missing-handler red.
