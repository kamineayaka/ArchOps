...
6. [docs/specs/unbound-identity-rebind.md](./docs/specs/unbound-identity-rebind.md) — 下一刀 Spec（未绑定观测候选 / 身份失联重绑；工单待 `/to-tickets`）  
7. [docs/dev-handoff.md](./docs/dev-handoff.md) — 进度与下一票  
8. Tickets: 竖切 [.scratch/vertical-slice-mvp/issues/](./.scratch/vertical-slice-mvp/issues/) 已 done；改策展 [.scratch/change-curated-draft/issues/](./.scratch/change-curated-draft/issues/)（**01–06 TDD-done**；本刀闭合）。下一刀 `.scratch/unbound-identity-rebind/`（Spec 已发布；工单待 `/to-tickets`）。不要默认 `/implement`
