...
| SSH | **Apache MINA SSHD**；每副本进程内连接池；计划互斥靠 Redis 锁；禁止旁路直连 |
| AI 出站 | Spring **WebClient** 调白名单外部 API（ADR-0041）；**禁止** LangChain 类重型编排框架 |
| 交付 | 单应用镜像 `archops:latest`（API + 前端静态）多副本 + `postgres` + `redis`；Agent 不进控制面默认 Compose |
| 入口 | v1 可用编排 LB / 宿主机端口；反向代理与统一 TLS **Later** |
| 服务拆分 | v1 单进程镜像（可多副本）；**允许 Later** 拆 worker/执行器等（须新 ADR） |
...
- MyBatis-Plus、Gradle、MINA SSHD、WebClient  
...
- 部署形态：`image: archops:latest`（可带仓库前缀）+ Postgres + Redis；副本数 ≥ 2 为设计前提。  
