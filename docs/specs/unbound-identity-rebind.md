...
**Status**: spec published；工单 01–07 已拆（**01–03 TDD-done**；frontier = **04**。tracker = `docs/agents/issue-tracker.md`）  
...
- **Next Matt step**: `/implement` `/tdd` **票 04**（frontier）。一次一张；不要重拆竖切 01–13 或改策展 01–06。不要加改策展 07。
...
- **Prompt for next chat**: 票 04 路径 [`.scratch/unbound-identity-rebind/issues/04-label-match-consume.md`](../../.scratch/unbound-identity-rebind/issues/04-label-match-consume.md)。进度见 [`docs/dev-handoff.md`](../dev-handoff.md)。Frontier = 未绑定 04。`/implement` 走 [`docs/agents/tdd.md`](../agents/tdd.md)。不要实现 05–07。
