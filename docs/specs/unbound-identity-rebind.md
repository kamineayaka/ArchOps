...
**Status**: spec published；工单 01–07 已拆（**01–02 TDD-done**；frontier = **03**。tracker = `docs/agents/issue-tracker.md`）  
...
- **Next Matt step**: `/implement` `/tdd` **票 03**（frontier）。一次一张；不要重拆竖切 01–13 或改策展 01–06。不要加改策展 07。
...
- **Prompt for next chat**: 票 03 开工入口 [`docs/implement-unbound-identity-rebind-03-prompt.md`](../implement-unbound-identity-rebind-03-prompt.md)。进度见 [`docs/dev-handoff.md`](../dev-handoff.md)。Frontier = `.scratch/unbound-identity-rebind/issues/03-itemized-create-and-bind.md`。`/implement` 走 [`docs/agents/tdd.md`](../agents/tdd.md)。
