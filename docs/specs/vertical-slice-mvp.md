...
- **Next Matt step**: 竖切 01–13 已闭合。改策展 Spec：[`change-curated-draft.md`](change-curated-draft.md)；工单 [`.scratch/change-curated-draft/issues/`](../../.scratch/change-curated-draft/issues/)（**01–04 TDD-done**；frontier = **05**）。不要重拆本 Spec。`/implement` 走 [`docs/agents/tdd.md`](../agents/tdd.md)。
