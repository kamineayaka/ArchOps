...
- **Next Matt step**: 另开对话 `/implement` 票 **05**（升级/空洞作废未完成草案）。票路径 [`.scratch/change-curated-draft/issues/05-void-draft-on-upgrade-hollow.md`](../../.scratch/change-curated-draft/issues/05-void-draft-on-upgrade-hollow.md)。开工 prompt：[`docs/implement-change-curated-draft-05-prompt.md`](../implement-change-curated-draft-05-prompt.md)。一次只做一张；不要重拆竖切 01–13。
...
- **Prompt for next chat**: 进度见 [`docs/dev-handoff.md`](../dev-handoff.md)。票 01–04 已 done。下一对话用 [`docs/implement-change-curated-draft-05-prompt.md`](../implement-change-curated-draft-05-prompt.md)。
