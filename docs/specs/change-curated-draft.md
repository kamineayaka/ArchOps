...
- **Next Matt step**: 改策展 01–06 已 TDD-done，**本刀闭合**。下一刀 Spec：[`unbound-identity-rebind.md`](unbound-identity-rebind.md)（已发布；工单待 `/to-tickets`）。不要加 07。不要重拆竖切 01–13。
...
- **Prompt for next chat**: 进度见 [`docs/dev-handoff.md`](../dev-handoff.md)。下一刀 Spec [`unbound-identity-rebind.md`](unbound-identity-rebind.md) 已发布；先 `/to-tickets`。票 01–06 已 done，本刀闭合。
