...
- **Next Matt step**: 另开对话 `/implement` 票 **03**（选改理想生成草案）。票路径 [`.scratch/change-curated-draft/issues/03-select-change-curated-draft.md`](../../.scratch/change-curated-draft/issues/03-select-change-curated-draft.md)。一次只做一张；不要重拆竖切 01–13。
...
- **Prompt for next chat**: 进度见 [`docs/dev-handoff.md`](../dev-handoff.md)。票 01–02 已 done（关闭建底覆盖；诊断改理想分叉）。
