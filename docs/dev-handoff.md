...
6. `docs/specs/unbound-identity-rebind.md`（未绑定 / 身份失联重绑 Spec；工单 01–07 已拆；**01–03 TDD-done**；编号 frontier = 04，**实现已暂停**）
...
10. `docs/audit-unbound-identity-rebind-01-03-prompt.md`（本阶段下一对话：Opus 5 只读合同审计）
...
| 未绑定 / 身份失联工单 | **01–03 TDD-done；编号 frontier = 04，实现已暂停** → 先 Opus 合同审计 01–03（[`docs/audit-unbound-identity-rebind-01-03-prompt.md`](audit-unbound-identity-rebind-01-03-prompt.md)）；工单 [`.scratch/unbound-identity-rebind/issues/`](../.scratch/unbound-identity-rebind/issues/)（不要写进 `change-curated-draft`） |
...
21. **暂停 04–07 实现。** 下一对话：**Claude Opus 5** 只读合同审计 01–03（跨票组合语义；不是单票 400 字 `/code-review`，不是 `/implement` 04）。开工 prompt：[`docs/audit-unbound-identity-rebind-01-03-prompt.md`](audit-unbound-identity-rebind-01-03-prompt.md)。审完由人决定先修 FIX-NOW 还是再开 04。票 04 仍是编号 frontier，但本阶段不要 `/implement`。不要加改策展 07。不要重拆竖切 01–13。不要做 05–07。  
...
未绑定 / 身份失联重绑（01–07 已拆；**01–03 TDD-done；编号 frontier = 04，实现已暂停**）：

```
01 推断失联 + 未绑定 upsert + 规范问法（TDD-done）
   ├→ 02 从不挂冲突的候选发草案（TDD-done） → 03 逐条新建/绑定（TDD-done） → （暂停）04 标签命中收尾 ─┐
 └→ 05 失联闸门修实际/改理想 ──────────────────────────────────────────────┴→ 06 HTTP tracer → 07 薄 UI
```

（一次只做一张。本阶段不做 04：先 Opus 合同审计 01–03。）
