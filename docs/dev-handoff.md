...
| 项 | 状态 |
|---|---|
| Gradle + Spring Boot + MyBatis-Plus + Flyway | 有；至 V14（草案条目接受/拒绝事件） |
...
| 改策展草案工单 | **进行中** → [`.scratch/change-curated-draft/issues/`](../.scratch/change-curated-draft/issues/)（**01–04 done**：关闭建底覆盖 + 诊断改理想分叉 + 选支出草案 + 逐条确认写入并立刻比对；05–06 `ready-for-agent`；从竖切 MVP 往上长，不重拆 01–13） |
...
11. ~~`/implement` 票 04~~：已完成（逐条确认写入并立刻比对；相等 → 待确认关闭，不自动关单）。  
12. **下一对话：`/implement` 票 05**（升级/空洞作废未完成草案）。票路径 [`.scratch/change-curated-draft/issues/05-void-draft-on-upgrade-hollow.md`](../.scratch/change-curated-draft/issues/05-void-draft-on-upgrade-hollow.md)。一次只做一张；不要顺手做 06。HTTP API 主接缝；勿用代码偷改领域合同。
...
01 关闭建底覆盖（done） ─────────────────┐
                                         │
02 诊断改理想分叉（done） → 03 选支出草案（done） → 04 逐条确认写入并立刻比对（done）
                                         │
                                         └→ 05 升级/空洞作废草案
                                              → 06 HTTP tracer（本刀定义完成）
```

（Frontier：05 的 blocker 04 已 done，`ready-for-agent`。一次只做一张。）
