...
| 改策展草案工单 | **进行中** → [`.scratch/change-curated-draft/issues/`](../.scratch/change-curated-draft/issues/)（**01–02 done**：关闭建底覆盖 + 诊断改理想分叉；03–06 `ready-for-agent`；从竖切 MVP 往上长，不重拆 01–13） |
...
10. **下一对话：`/implement` 票 03**（选改理想生成草案）。票路径 [`.scratch/change-curated-draft/issues/03-select-change-curated-draft.md`](../.scratch/change-curated-draft/issues/03-select-change-curated-draft.md)。开工提示词 [`docs/implement-change-curated-draft-03-prompt.md`](implement-change-curated-draft-03-prompt.md)。一次只做一张；不要顺手做 04–06。HTTP API 主接缝；勿用代码偷改领域合同。
...
01 关闭建底覆盖（done） ─────────────────┐
                                         │
02 诊断改理想分叉（done） → 03 选支出草案（≥2 条，不写策展、不出计划）
                                         │
...
（Frontier：03 的 blocker 02 已 done，`ready-for-agent`。一次只做一张。）
