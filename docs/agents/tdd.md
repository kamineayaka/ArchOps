...
The ticket's spec `Testing seams (confirmed)` line is the seam. For `unbound-identity-rebind`, `change-curated-draft`, the closed vertical-slice MVP, `conflict-upgrade-void-plans`, and `plan-step-assertion` that is the control-plane public HTTP API. For `control-plane-executor` the **primary** seam is still that HTTP API; the spec also requires 执行引擎 `grpc.health.v1` and a narrow mTLS-negative gRPC call. `plan-step-assertion` rides the same HTTP primary seam (gRPC fields follow the HTTP story; proto tests are not the definition of done). Gradle/`MockMvc` and Compose `bootRun`+`curl` are the same HTTP seam. Thin UI is demo-only and is not an automated seam.
