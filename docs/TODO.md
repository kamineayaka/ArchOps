...
| P0-2 | **单元测试：`LlmRuntimeFactory`** | OpenAI / Anthropic 请求体构造、API Key 加解密 | mock 或快照测试 |
| P0-3 | **集成测试：AI Provider CRUD** | `@SpringBootTest` + Testcontainers Postgres | 创建/更新/删除/脱敏/默认 Provider 约束 |
| P0-4 | **CI 全绿验证** | 大改后首次完整 `mvn verify` + `npm run build` | GitHub Actions 通过 |
...
| # | 任务 | 说明 | 完成标准 |
|---|------|------|----------|
| P1-1 | **英文 UI（`en-US`）** | 新增 `frontend/src/locales/en-US.ts`，顶栏语言切换 | 主要页面双语可切换 |
