...
7. 当前工单：见 `docs/dev-handoff.md`；竖切 `.scratch/vertical-slice-mvp/issues/` 已 done；改策展 `.scratch/change-curated-draft/issues/`（**01–02 TDD-done**；03 `ready-for-agent` TDD redo；**frontier = 03**；04–06 仍被挡住）
...
- 下一刀 Spec：`docs/specs/change-curated-draft.md`（已发布）；工单 `.scratch/change-curated-draft/issues/`（01–06 已拆）。**01–02 TDD 重做已 done**（关闭建底覆盖；诊断改理想分叉）。frontier = **03**（选改理想生成草案，TDD 重做）。04 被 03 挡住。
