...
8. 当前工单：见 `docs/dev-handoff.md`；未绑定 **01–03 TDD-done**；编号 frontier = 04，**实现已暂停**。下一对话先做 01–03 合同审计（[`docs/audit-unbound-identity-rebind-01-03-prompt.md`](docs/audit-unbound-identity-rebind-01-03-prompt.md)），不要 `/implement` 04–07。竖切与改策展工单包已闭合。不要加改策展 07
...
- 下一刀 Spec：[`docs/specs/unbound-identity-rebind.md`](docs/specs/unbound-identity-rebind.md)；工单 `.scratch/unbound-identity-rebind/issues/`（01–07 已拆）。**01–03 TDD-done。编号 frontier = 04，实现已暂停。** 先 Opus 合同审计 01–03（[`docs/audit-unbound-identity-rebind-01-03-prompt.md`](docs/audit-unbound-identity-rebind-01-03-prompt.md)）。不要 `/implement` 04–07。不要加改策展 07。
- `/implement` 必须走 TDD overlay：[`docs/agents/tdd.md`](docs/agents/tdd.md)
- 不需要再开技术选型或推倒栈，除非用户明示新 ADR

## 7. 云端提示词建议（用户可贴）

改策展 01–06 已闭合。未绑定票 **01–03 TDD-done**；**暂停 04–07 实现**。下一对话用 Claude Opus 5 做 01–03 合同审计，prompt：[`docs/audit-unbound-identity-rebind-01-03-prompt.md`](docs/audit-unbound-identity-rebind-01-03-prompt.md)。不要 `/implement` 04。不要加改策展 07。不要用下面这句短指令去开工 04。

> 读 AGENTS.md、CONTEXT.md、ADR-0043 与 `docs/specs/unbound-identity-rebind.md`。只做 `docs/audit-unbound-identity-rebind-01-03-prompt.md` 里的只读合同审计。不要实现 04–07。不要复活 Vue/JPA/Neo4j/Maven/LangChain。
