...
8. 当前工单：见 `docs/dev-handoff.md`；**未绑定 01–07 + 08 已闭合**（07 = 薄 UI）。竖切与改策展工单包已闭合。不要加改策展 07。票 09 待人排期，不要自动做。不要发明未绑定 10。
...
- 下一刀 Spec：[`docs/specs/unbound-identity-rebind.md`](docs/specs/unbound-identity-rebind.md)；工单 `.scratch/unbound-identity-rebind/issues/`（01–07 已拆，审计后补 08 / 09）。**01–07 + 08 已闭合**（07 = 薄 UI）。`/implement` 走 TDD overlay，一次一张。不要加改策展 07。不要自动做票 09。票 09（审计 C-1）待人排期。不要发明未绑定 10。
...
- 代码 vs ADR-0044 只读审计：[`.scratch/unbound-identity-rebind/audit-code-vs-adr-0044.md`](.scratch/unbound-identity-rebind/audit-code-vs-adr-0044.md)。A2 = 票 05；A3 = 票 09；A1 另开；B1–B5 禁止写入未绑定刀。票 07 已交付演示层。
...
改策展 01–06 已闭合。未绑定票 **01–07 + 08 已闭合**（07 = 薄 UI）。一次一张 `/implement` `/tdd`，不要加改策展 07。票 09（失联叠加心跳超时的问法；审计 C-1）待人排期，不要自动做。不要发明未绑定 10。不要用下面这句短指令替代票内循环纪律。

> 读 AGENTS.md、CONTEXT.md、ADR-0043 与 `docs/specs/unbound-identity-rebind.md`。不要默认做票 09。不要复活 Vue/JPA/Neo4j/Maven/LangChain。
