...
8. 当前工单：见 `docs/dev-handoff.md`；**frontier = 未绑定票 03**（01–02 TDD-done；开工 prompt [`docs/implement-unbound-identity-rebind-03-prompt.md`](docs/implement-unbound-identity-rebind-03-prompt.md)）。竖切与改策展工单包已闭合。不要加改策展 07
...
- 下一刀 Spec：[`docs/specs/unbound-identity-rebind.md`](docs/specs/unbound-identity-rebind.md)；工单 `.scratch/unbound-identity-rebind/issues/`（01–07 已拆）。**01–02 TDD-done。frontier = 03**（逐条新建/绑定）。开工 prompt：[`docs/implement-unbound-identity-rebind-03-prompt.md`](docs/implement-unbound-identity-rebind-03-prompt.md)。`/implement` 走 TDD overlay，一次一张。不要加改策展 07。
- `/implement` 必须走 TDD overlay：[`docs/agents/tdd.md`](docs/agents/tdd.md)
- 不需要再开技术选型或推倒栈，除非用户明示新 ADR

## 7. 云端提示词建议（用户可贴）

改策展 01–06 已闭合。未绑定票 **01–02 TDD-done**；**frontier = 03**。一次一张 `/implement` `/tdd`，不要加改策展 07。新对话请粘贴 [`docs/implement-unbound-identity-rebind-03-prompt.md`](docs/implement-unbound-identity-rebind-03-prompt.md) 的复制区，不要用下面这句短指令替代那份循环纪律。

> 读 AGENTS.md、CONTEXT.md、ADR-0043 与 `docs/specs/unbound-identity-rebind.md`。只做 `.scratch/unbound-identity-rebind/issues/03-itemized-create-and-bind.md`。开工入口见 `docs/implement-unbound-identity-rebind-03-prompt.md`。不要默认做 04–07。不要复活 Vue/JPA/Neo4j/Maven/LangChain。
