...
public class ArchitectureViewService {
    ...
    @Transactional
    public ArchitectureViewResponse buildView(List<Long> targetAssetIds) {
        return buildView(targetAssetIds, null);
    }

    /**
     * Build architecture view for conversation targets plus optional extra partition keys
     * (e.g. asset:{elementId}, cluster:{elementId}, tag:{slug} from graph neighborhood).
     */
    @Transactional
    public ArchitectureViewResponse buildView(List<Long> targetAssetIds, Collection<String> extraPartitionKeys) {
        ...
        if (extraPartitionKeys != null) {
            for (String key : extraPartitionKeys) {
                if (key == null || key.isBlank()) {
                    continue;
                }
                try {
                    String normalized = PartitionKeys.normalize(key);
                    PartitionKeys.validate(normalized);
                    keys.add(normalized);
                } catch (Exception ignored) {
                    // skip invalid scope keys from graph suggestions
                }
            }
        }
    ...
    public String toPromptSnippet(ArchitectureViewResponse view) {
        ...
        PartitionDetailResponse global = view.partitions().stream()
                .filter(p -> PartitionKeys.isGlobal(p.partitionKey()))
                .findFirst()
                .orElse(null);
        if (global != null) {
